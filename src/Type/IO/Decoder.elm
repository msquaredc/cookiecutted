module Type.IO.Decoder exposing (..)
...
result _ val = Json.Decode.map (\x -> Ok x) val

array : Decoder a -> Decoder (Array a)
array = Json.Decode.array

entity : a -> Decoder a
entity = Json.Decode.succeed

attribute : String -> Decoder a -> Decoder (a -> b) -> Decoder b
attribute name child parent = parent |> required name child

reference : String -> Decoder a -> Decoder (a -> b) -> Decoder b 
reference = attribute

references : String -> Decoder a -> Decoder (List a -> b) -> Decoder b
references name child parent = parent |> required name (Json.Decode.list child)
