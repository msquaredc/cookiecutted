                                ...
                                [ cell [{- LG.span12 -}] <|
                                    ...
                                    , TextField.filled
                                        (TextField.config
                                            ...
                                            -- TODO: Uncomment or Reimplement |> TextField.setFullwidth True
    ...
    if model.short == Just itid then
        case Maybe.map .value <| Maybe.andThen (\x -> Dict.get (unbox x) db.input_types) model.short of
            Just (IT.ShortAnswer short) ->
                ...
                , p []
                    [ text <| "Min Length: " ++ (String.fromInt short.minLength)
                    , Slider.slider
                        (Slider.config
                            |> Slider.setValue (toFloat short.minLength)
                ...
                , p []
                    [ text <| "Max Length: " ++ (String.fromInt short.maxLength)
                    , Slider.slider
                        (Slider.config
                            |> Slider.setValue (toFloat short.maxLength)
        ...
        if model.long == Just itid then
            ...
            Just (IT.LongAnswer long) ->
                ...
                , p []
                    [ text <| "Min Length: " ++ (String.fromInt long.minLength)
                    , Slider.slider
                        (Slider.config
                            |> Slider.setValue (toFloat long.minLength)
                            --                        |> Slider.setMax (Maybe.map toFloat short.maxLength)
                            |> Slider.setOnInput
                                (\x ->
                                    Match.setField
                ...
                , p []
                    [ text <| "Max Length: " ++ (String.fromInt long.maxLength)
                    , Slider.slider
                        (Slider.config
                            |> Slider.setValue (toFloat long.maxLength)
                            --                        |> Slider.setMin (Maybe.map toFloat short.minLength)
                            |> Slider.setOnInput
                                (\x ->
                                    Match.setField
