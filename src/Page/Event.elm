...
viewList elements onClick nameGetter =
    let
        mlist =
            List.map (\( x, y ) -> listItem (MLItem.config |> MLItem.setOnClick (onClick (unbox x))) [ graphic [] [ identicon "100%" (unbox x) ], text <| nameGetter ( x, y ) ]) elements
    in
    case mlist of
        f :: r ->
            list MList.config f r
                                ...
                                (\( questionary_id, _ ) ->
                                    DataTable.cell []
                                        [ let
                                            ...
                                            q_ids =
                                                List.map (\( qid, _ ) -> qid) questions
                                          in
                                          Button.unelevated
                                            (Button.config |> Button.setOnClick (Msg.FollowSubpage Db.EventType (unbox event_id) [ "answer" ] [ Url.Builder.string "qid" (unbox questionary_id), Url.Builder.string "tsid" (unbox test_subject_id) ]))
                                          <|
                                            (String.fromInt <| List.length answers)
                                                ++ "/"
